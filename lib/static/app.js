...
function analyzeEncodingProba(view) {
    ...
    let hasControlBytes = false;
    let hasUtf8Pattern = true;
    ...
    for (let i = 0; i < view.length; i++) {
        ...
        if (byte >= 0x80) hasHighBytes = true;
        ...
        if (byte >= 0x80) {
            if ((byte & 0xC0) === 0x80) {
                ...
                if (continuationCount > maxContinuation) maxContinuation = continuationCount;
                continuationCount = 0;
                if ((byte & 0xE0) === 0xC0) continuationCount = 1;
                else if ((byte & 0xF0) === 0xE0) continuationCount = 2;
                else if ((byte & 0xF8) === 0xF0) continuationCount = 3;
                else hasUtf8Pattern = false;
            }
        }
    }
    if (continuationCount > maxContinuation) maxContinuation = continuationCount;
    ...
    // 决策
    if (hasControlBytes && !hasUtf8Pattern) {
        return 'GBK'; // 常见的中文编码
    }
    
    if (hasUtf8Pattern && maxContinuation <= 3) {
        return 'UTF-8';
    }
