...
window.previewFile = function(href, mediaType, name) {
    // 获取文件信息
    const fileItem = document.querySelector(`[data-href="${CSS.escape ? CSS.escape(href) : href.replace(/"/g, '\\"')}"]`)
        || document.querySelector(`[data-href="${href}"]`);
    ...
    if (!mediaType || mediaType === 'text' || isTextFile(name)) {
        ...
        const img = document.querySelector(`[data-viewer-image="${href}"]`)?.querySelector('.viewer-image')
            || fileItem?.querySelector('.viewer-image');
        if (img) {
