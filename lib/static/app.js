...
class WindowsExplorer {
    ...
    getShareUrl(href) {
        ...
        return `${protocol}//${ip}:${port}${href}`;
    }

    shareCurrentDirectory() {
        const shareUrl = this.getShareUrl(window.location.pathname);
        this.showQr(shareUrl, '当前目录');
    }

    shareFile(href, name) {
        const shareUrl = this.getShareUrl(href);
        this.showQr(shareUrl, name);
    }
