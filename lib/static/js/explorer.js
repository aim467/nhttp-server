...
class WindowsExplorer {
    ...
    buildSearchResultItemHtml(item, isList) {
        ...
        const thumb = mediaType === 'image'
            ? `<div class="file-thumb has-thumb"><img src="${hrefAttr}" alt="" class="file-thumb-img" loading="lazy" onerror="this.classList.add('is-broken');this.nextElementSibling.classList.add('is-fallback-visible');"><i class="bi ${icon.icon} ${icon.color} file-thumb-fallback"></i></div>`
