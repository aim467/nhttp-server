...
function showMediaPreview(href, mediaType, fileName) {
    ...
    if (mediaType === 'image') {
        modalBody.innerHTML = `<img src="${escapeAttr(href)}" alt="${escapeAttr(fileName)}" class="img-fluid">`;
        modal.show();
        ...
        modalBody.innerHTML = `<div class="text-center p-4">
                     ...
                     <h5 class="text-white mb-3">${escapeHtml(fileName)}</h5>
                     <audio controls class="w-100">
                       <source src="${escapeAttr(href)}">
                       您的浏览器不支持音频播放。
                     </audio>
                   </div>`;
        modal.show();
    } else if (mediaType === 'pdf') {
        modalBody.innerHTML = `<iframe class="pdf-preview-frame" src="${escapeAttr(href)}#toolbar=1" title="${escapeAttr(fileName)}"></iframe>`;
        modal.show();
    }
}
...
function renderPreviewContent(content, fileSize) {
    ...
    displayLines.forEach((line, index) => {
        ...
        // 语法高亮内部已 escape；普通文本在此 escape
        const highlightedLine = isCodeFile
...
// 语法高亮：按注释/字符串/关键字/数字分词，逐段 escape 后包 span
function highlightSyntax(line, ext) {
    const family = getHighlightFamily(ext);
    const keywords = KEYWORDS_BY_FAMILY[family] || KEYWORDS_BY_FAMILY.js;

    let commentSrc = '\\/\\/.*';
    if (family === 'hash') commentSrc = '#.*';
    else if (family === 'html') commentSrc = '<!--[\\s\\S]*?-->';
...
const KEYWORDS_BY_FAMILY = {
    js: 'const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|class|import|export|from|async|await|try|catch|finally|throw|new|this|super|extends|typeof|instanceof|null|undefined|true|false|of|in|yield|default|static|get|set',
    py: 'def|class|return|if|elif|else|for|while|import|from|as|try|except|finally|raise|with|yield|lambda|True|False|None|and|or|not|in|is|pass|break|continue|global|nonlocal|async|await',
    java: 'public|private|protected|class|interface|extends|implements|return|if|else|for|while|do|switch|case|break|continue|new|this|super|static|final|void|int|long|boolean|true|false|null|try|catch|finally|throw|throws|import|package',
    go: 'func|package|import|return|if|else|for|range|switch|case|break|continue|go|defer|chan|map|struct|interface|type|var|const|true|false|nil|select|default',
    rust: 'fn|let|mut|const|struct|enum|impl|trait|pub|use|mod|return|if|else|match|loop|while|for|in|break|continue|true|false|Self|self|async|await|crate|super|where|type',
    css: 'important|from|to|and|or|not|only',
    sql: 'SELECT|FROM|WHERE|AND|OR|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|INDEX|JOIN|LEFT|RIGHT|INNER|OUTER|ON|AS|ORDER|BY|GROUP|HAVING|LIMIT|NULL|TRUE|FALSE|DISTINCT',
    shell: 'if|then|else|elif|fi|for|while|do|done|case|esac|function|return|in|export|local|readonly|true|false',
    html: '',
    json: 'true|false|null',
    hash: 'True|False|None|true|false|null|yes|no',
    plain: ''
};

function getHighlightFamily(ext) {
    const map = {
        js: 'js', ts: 'js', jsx: 'js', tsx: 'js', mjs: 'js', cjs: 'js', vue: 'js',
        py: 'py',
        ...
        yml: 'hash', yaml: 'hash', rb: 'hash', php: 'py',
        c: 'java', cpp: 'java', h: 'java', hpp: 'java',
        md: 'plain', txt: 'plain', log: 'plain', ini: 'hash', conf: 'hash'
    };
    return map[ext] || 'js';
}
